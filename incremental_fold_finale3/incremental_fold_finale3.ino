 ...
 *     k        토크 복구 — ★하드웨어 에러가 있으면 리부트로 지운 뒤 복구한다
...
float LINE_C   = 0;    // [c0:-1.11f ] 실측② 안정모드선 절편 [deg] ±0.86 ⚠미확정 (문서 69 §6)
float FOLD_SIGN = +1.0f;    // [sgn] 접기 방향 ±1 — ⚠ 헤더의 바닥 시험으로 확인할 것

// ---- 제어 ----
float GAMMA       = 12.0f;  // [gam ] ★접기 이득 γ — 줄 위 노브. Δδ = ρ·γ·Â
...
float CUE_TH      = 1.0f;   // [cue 0.3f] 놓기신호 문턱 [deg] (문서 50)
...
// 모터가 부팅 직후처럼 되도록 다시 설정한다 (리부트 뒤에는 RAM 값이 날아간다)
void motorReinit() {
...
// ★핵심 — 다이나믹셀은 하드웨어 에러가 뜨면 토크를 스스로 끊고,
//   그 뒤 torqueOn 을 아무리 보내도 안 걸린다. 리부트로 플래그를 지워야 한다.
//   반환값: 에러가 있어서 리부트했으면 true
bool motorClearError() {
  if (!motor_ok) return false;
  int32_t e = dxl.readControlTableItem(HARDWARE_ERROR_STATUS, DXL_ID);
  if (e <= 0) return false;                       // 0 = 정상, 음수 = 읽기 실패

  Serial.print("!! 모터 하드웨어 에러 0x"); Serial.println(e, HEX);
  printHwError(e);
  Serial.println("   -> 리부트로 지운다 (torqueOn 만으로는 절대 안 풀린다)");

  dxl.reboot(DXL_ID, 200);
  delay(1200);                                    // 리부트에 시간이 걸린다

  motor_ok = false; dxl_fail_n = 0;
  for (int i = 0; i < 5 && !motor_ok; i++) {
    if (dxl.ping(DXL_ID)) motor_ok = true; else delay(200);
  }
  if (!motor_ok) { Serial.println("<< 리부트 후에도 응답 없음 — 전원을 껐다 켤 것 >>"); return true; }

  dxl_err = false;
  motorReinit();
  Serial.println("# 리부트 완료 — 토크 복구됨");
  Serial.println("#   ★영점이 무효다. z 를 다시 누를 것 (home_tick 을 새로 잡았다)");
  return true;
}
...
void printStatus() {
  ...
  if (motor_ok) {
    ...
    Serial.print(v / 10.0f, 1); Serial.println(" V");
    if (v > 0 && v < 110) Serial.println("  ★전압이 낮다 — 배터리 확인 (11.0 V 미만)");
  }
...
void printAllParams() {
  ...
  Serial.println("-- 제어 --");
  for (int i = 5; i < 18; i++) printParam(i, true);
  Serial.println("-- 기록·모터·감시 --");
  for (int i = 18; i < N_PARAM; i++) printParam(i, true);
...
void handleLine(char* s) {
  ...
  switch (tok[0]) {
    ...
    case 'k':
      ...
      if (!motorClearError()) {                   // 에러가 있었으면 리부트가 토크까지 복구한다
        torqueRestoreHere();
