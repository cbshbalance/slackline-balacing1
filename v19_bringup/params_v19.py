...
ROPE = dict(
    ...
    # ★2026-08-05 정정(프로젝트 문서 33, 8/1 발견 반영): 위 식의 I_exp=0.0190 은
    #   실험① 당시 매달림 구성 가정의 관성이고, 확정 줄 관성은 I_r=4.50e-3 (doc 21).
    #   감쇠 계수는 I에 비례하므로 4.50e-3/0.0190 배로 축소:
    #   점성 0.0014 → 3.3e-4, 건마찰 0.0013 → 3.1e-4.
    #   (비동기 FWE 프리셋은 이미 정정값 사용 — 게인 계산 경로만 낡아 있었음)
    c_phi = dict(v=3.3e-4, MEASURED=True, how="2026-08-05 정정: 실험① ζ=0.0076 × ω=4.852 × 확정 I_r=4.50e-3 (구 0.0014는 I_exp 0.019 오적용)"),
    # 건마찰(쿨롱): 실험① 반주기당 0.34° 감소 → τ_c = 0.069×I_r. 소진폭에서 등가
    #   점성감쇠 c_eq = 4τ_c/(π·ω·A) 가 지배.
    tau_coulomb = dict(v=3.1e-4, MEASURED=True, how="2026-08-05 정정: 0.34°/반주기 → 0.069 × 확정 I_r=4.50e-3 (구 0.0013은 I_exp 0.019 오적용)"),
)
