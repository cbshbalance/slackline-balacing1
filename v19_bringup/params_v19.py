...
# ============================================================
# [5] ★실측 정본 세트 (2026-07-30 ~ 08-06 실측)
#     2026-08-07 "Sync restored" 커밋 때 이 파일에서 소실됐던 값들의 복원본.
#     기본값(위 BODY/ROPE)은 설계추정치 그대로 두고, 여기에 따로 보관한다.
#     쓰는 법:  SimEngine.nominal.update(params_v19.MEASURED_SET)
#
#     근거
#       · 몸체 질량·길이·CoM·복합진자 관성 : 문서 40 §4-2 (7/30-31 실측)
#       · I_r (줄 관성)                     : 문서 21 정본 (부품 저울)
#       · S_r (줄 정적모멘트)               : 문서 16 자유흔들기 omega_n = 4.86 rad/s 실측
#                                             -> S_r = omega_n^2 * I_r / g = 0.010835
#                                             (교차검증식 omega^2*I_r = S_r*g 를 역으로 사용)
#       · c_phi (줄 점성)                   : 문서 16 zeta = 0.0076  ->  c_phi = 0.074 * I_r
#       · 줄 건마찰(시뮬 real.f_phi)        : 문서 16 0.34 deg/반주기 -> tau_c = 0.069 * I_r
# ============================================================
MEASURED_SET = dict(
    m1    = 0.1094,     # [kg] 하체 저울
    m2    = 0.6406,     # [kg] 상체 저울 (장비 매립 후)
    L1    = 0.259,      # [m]  발축 -> 힙축
    L2    = 0.375,      # [m]  힙축 -> 상체 끝
    ell1  = 0.084,      # [m]  발축 -> 하체 CoM (균형점)
    ell2  = 0.1917,     # [m]  힙축 -> 상체 CoM (균형점)
    I1_cm = 8.85e-4,    # [kg m^2] 하체 복합진자
    I2_cm = 2.116e-2,   # [kg m^2] 상체 복합진자
    I_r   = 4.50e-3,    # [kg m^2] 줄 관성 (부품표 실측)   ★부품표 계산을 덮어쓴다
    S_r   = 1.0835e-2,  # [kg m]   줄 정적모멘트 (omega_n 실측에서)  ★〃
    c_phi = 3.33e-4,    # [N m s/rad] 줄 점성감쇠
)
# 시뮬 real 쪽에 같이 넣어야 하는 값 (플랜트 마찰)
MEASURED_REAL = dict(
    f_phi = 3.105e-4,   # 줄 건마찰(쿨롱) — 0.34 deg/반주기 실측
)
