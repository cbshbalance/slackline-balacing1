...
float DCMD_CAP     = 40.0f;        // δ 명령 상한 [°] = 유일한 캡 (기구한계 55 에서 15° 여유)
                                   //   기구 간섭(케이블·프레임)이 없다고 확인되면 55 까지 올릴 수 있다.
                                   //   시뮬은 55 로 돌고, 실측 파라미터·초기 4° 에서 최대 유지각 38° 였다.
float STEP_CAP     = 40.0f;        // 증분 1회 상한 [°] = DCMD_CAP (관절 한계에 맞춤)
                                   // ★2026-08-14 변경: 22 -> 40. 캡을 '기구 한계' 하나로 통일.
                                   //   근거(v19_sim, 실측 파라미터·γ=10 고정, 30s 상한):
                                   //     초기각 3.0° -> 필요한 첫 증분 28.6°
                                   //     초기각 4.0° -> 필요한 첫 증분 38.0°
                                   //     STEP_CAP 22 로 자르면 초기 4°에서 1.2s 만에 낙하,
                                   //     캡을 풀면 30s 완주. 즉 22 는 '첫 접기'를 잘라버린다.
                                   //   비례식의 논리는 "A 가 크면 그만큼 크게 접는다" 이므로
                                   //   캡에 걸린다 = 필요한 만큼 못 접는다 = 못 잡는다.
                                   //   글리치 방어는 앞단(A 클램프 ±60, 정상범위 게이트,
                                   //   각속도 물리클램프, A 저역필터)이 담당한다.
                                   //   (이력: 8/6 r3 에 12 -> 22. 12 는 추격 실패였다)
uint32_t FOLD_LOCK_MS = 120;       // 증분 후 잠금 (8/6 r3: 큰 증분+짧은 주기 = 유효 접기속도 180°/s+)
float FALL_ALPHA_DEG  = 25.0f;     // |α| 초과 시 낙하 판정 → 토크 컷
// ★★TODO(2026-08-07, 프로젝트 문서 40): 250 → 600 으로 올릴 것. 토크·속도 수지 재계산 결과
//   줄 위 유효관성 0.0059 kg·m²(지그 0.0477의 1/8 — 접기 반작용으로 하체·줄이 따라 움직임)에서
//   acc250이 쓰는 토크는 0.55 N·m = 스톨 3.0의 18%뿐. 즉 우리 설정이 모터보다 먼저 병목이다.
//   acc600 → 필요토크 1.33 N·m(44%), 20° 접기 122→81ms, 회복 천장 β 2.8°→3.6°.
//   ⚠벤치에서 잡을 사람 두고 적용 — 결합부 7Hz 진동을 더 세게 두드린다.
int PROF_ACC_UNIT = 250;           // ≈5360°/s² (★위 TODO 참조: 600 권장, 미적용)
