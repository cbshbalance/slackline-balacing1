...
// ====== Frame parameters - v6.3 ============================================
// Design notes live HERE, above the customizer section, because OpenSCAD's
// Customizer swallows the comment block preceding a variable and shows it as
// that variable's description. Below this banner every parameter gets exactly
// one short trailing comment.
//
// WHY 60 ALONG Y: the top Y rails were deleted (2026-07-11) for swing
//   clearance, so every post is a bare cantilever in Y. Depth cubed -> the
//   60 mm face belongs on Y. X sway still has its two X-Z portal frames.
// HARD CONSTRAINT: front/rear post CENTRE spacing = 580 mm, fixed by the
//   carbon diagonal length. frame_y therefore tracks post_y automatically.
// THE DECK: a mid-height X beam pair spanning the full depth. Upper posts sit
//   on it, legs hang below it, die-cast angle brackets hold both -> pulling
//   the legs off is 8 bolts. The deck is also the base frame of the short
//   build, and the Y rails relocate onto it in that mode.
// NOTHING may live between the deck and the pivot: that whole volume is swept
//   by the hanging robot. Checked in the console echo.

/* [Frame - profile] */
post_x = 30;               // post cross-section along X (3060) [mm]
post_y = 60;               // post cross-section along Y - the stiff way [mm]
xr_y = 60;                 // X rail width along Y (flush with post) [mm]
xr_z = 30;                 // X rail height along Z [mm]
yr_x = 30;                 // Y rail width along X [mm]
yr_z = 60;                 // Y rail height along Z - 3060 ON EDGE [mm]
slot_pitch = 30;           // T-slot cell pitch of the 30-series profile [mm]
prof_ctr_y = 580;          // BEARING centre spacing - FIXED by carbon tube [mm]
frame_x = 600;             // depth, swing direction [mm]

/* [Frame - heights] */
auto_height = true;        // derive the two heights from the clearances below
design_hang = 631;         // WHICH build to draw: hang_short or hang_tall [mm]
hang_short = 631;          // short robot - the A post set [mm]
hang_tall = 800;           // tall robot  - the B post set [mm]
share_posts = true;        // A-build leg = B-build gantry post (one stock length)
clear_stand = 50;          // SHORT build, standing: hardware to floor [mm]
clear_fall = 50;           // LAB build, fallen and hanging: head to floor [mm]
clear_swing = 60;          // radial margin, swept robot to the floor Y rail [mm]
upper_h_man = 521;         // manual deck-top to post-top, if auto_height off
legs_h_man = 778;          // manual floor to deck underside, if auto_height off
expo_mode = false;         // short build: legs and floor rails removed
mount_rise = 25;           // pivot axis above the top rail top face [mm]

/* [Frame - gantry] */
show_gantry = true;        // top stiffening gantry (removable)
gantry_phi = 60;           // swing angle the gantry must clear, ANY lean [deg]
gantry_clear = 80;         // margin above the reach envelope [mm]

/* [Frame - brackets] */
show_brackets = true;      // draw the die-cast angle brackets
bracket_L = 30;            // 30-series die-cast angle: leg length [mm]
bracket_w = 30;            // width - two of these cover a 60 mm face [mm]
bracket_t = 7;             // thickness [mm]
bracket_bolt_d = 6.6;      // M6 clearance [mm]

/* [Hidden] */
// *** T-SLOT REALITY (user catch 2026-08-12) ***
// A 3060 has NO slot on the centreline of its 60 mm face - the two cells put
// slots at 15 and 45 from the edge. The pivot bracket must therefore bolt to
// the INNER cell, at xr_slot_in from the rail's outer face, and the bearing
// centre has to sit on that same line. Keeping the 580 mm bearing spacing then
// forces frame_y = 580 + 2*45 = 670 (was 640 when I wrongly used the profile
// centreline). sag and upper_w are unaffected - only the posts move outboard.
xr_slot_in = xr_y - slot_pitch/2;              // 45 - inner T-slot centreline
frame_y = prof_ctr_y + 2 * xr_slot_in;         // 670
xrail_len = frame_x - 2 * post_x;
yrail_len = frame_y - 2 * post_y;   // Y rails butt into the POSTS
axis_x = frame_x / 2;
top_mount_front_y = xr_slot_in;
top_mount_rear_y  = frame_y - xr_slot_in;
...
// v5.5 X-ONLY OPEN FACE (CoM request 2026-07-25): v5.4's three-side
// bay (X- wall + both Y-wall front strips removed) pushed the shell
// CoM to x +7.4 - too far behind the swing centerline for the user's
// taste. v5.5 RESTORES both Y walls ("z축 방향의 면들") to full
// section and keeps ONLY the X- face open, at the v5.3 full interior
// width (no lips): opening 85 in the 90 zone (vs the 75 board), 93 in
// the head. Shell CoM comes back toward center (~x +6) and the closed
// box section (both Y walls + X+ wall + bulkhead) is fully restored.
// Trade-off (accepted): in-plane edge plugs on the board's Y edges
// have no straight lateral path again - side cables approach at an
// angle through the wide front opening.
lip_ret_y = 0;             // v5.3+: no lips - fully open X- face
// v5.5: the v6-era POWER-TOGGLE SLOT in the shelf front edge is
// RETIRED. The shelf keeps only two side rails (see shelf_rail_w) and
// its whole middle is open - that opening IS the switch bay: the
// power toggle / OpenCR top-edge switch sits in the open middle,
// reached from the front, wiring drops straight to the board top.
// (tsl_* params kept for reference only - no geometry uses them.)
tsl_w = 13;                // (retired v5.5) toggle slot width along Y
tsl_back_x = 13;           // (retired v5.5) slot depth from shelf front
tsl_y = -20;               // (retired v5.5) slot center Y
...
// v5.5 SIDE-RAIL SHELF: the full shelf plate is replaced by two side
// rails along the Y walls ("z축 방향의 면들 쪽 벽만 유지") - the whole
// middle between them is OPEN. That middle opening is the SWITCH BAY
// (power toggle sits there, reached from the front) and a straight
// wiring drop from the battery to the board-top edge (25 mm cable
// room). The 84 mm pack lies across Y and rests on both rails: Y-
// end full on the rail (butted on the Y- head wall), Y+ end over the
// rail next to the lead bay. The bulkhead top edge still props both
// rails from below (|y| 33..44). The old central strap slots and the
// toggle slot lived in the removed middle - straps move onto the
// rails (one pair per rail), toggle slot retired (see tsl_*).
shelf_rail_w = 15;         // each side rail width along Y (middle
                           // opening = usp_y_head - 2 - 2*15 = 66)
strap_slot_w = 3;
strap_slot_l = 8;          // v5.5: 16 -> 8, fits inside a 15 rail
...
c_joint   = [0.35, 0.45, 0.62];
c_seg_leg = [0.42, 0.56, 0.68];   // stage 1 - legs      (slate blue)
c_seg_mid = [0.82, 0.83, 0.85];   // stage 2 - mid posts (light aluminium)
c_seg_top = [0.62, 0.74, 0.52];   // stage 3 - gantry posts (sage)
...
// ============ DERIVED HEIGHTS - v6.5 =========================================
// Order matters: OpenSCAD evaluates file-scope assignments top to bottom, so
// everything a formula needs must already be assigned above it.
ankle_drop = carrier_boss_d / 2;    // carrier boss below the ankle axis (22)
gantry_h   = max(xr_z, yr_z);       // gantry rectangle depth in Z
post_cap   = 40;                    // post left proud above the gantry
gantry_len = design_hang;           // the frame follows design_hang

// Everything is rounded UP to 5 mm so the cut list has no decimals. Rounding
// up only ever adds clearance.
function r5(x) = ceil(x / 5) * 5;
upper_h = auto_height ? r5(clear_stand + sag + ankle_drop - xr_z - mount_rise)
                      : upper_h_man;

// GANTRY HEIGHT - REACH ENVELOPE, not the upright head. What strikes the
// gantry Y beam is the MIDDLE of a leaning robot: at phi=90 a lean of only
// -23..-27 deg puts the torso through it. So for every swing angle take the
// highest point the robot can reach ANYWHERE inside the Y beam's X band -
// a disc of radius design_hang about the ankle, clipped to that band.
function g_ax(ph)   = axis_x + sag * sin(ph);
function g_dx(ph)   = max(0, g_ax(ph) - frame_x, (frame_x - yr_x) - g_ax(ph));
function g_rise(ph) = -sag * cos(ph)
                    + sqrt(max(0, pow(gantry_len, 2) - pow(g_dx(ph), 2)));

// legs_h must satisfy BOTH: the fallen robot clears the floor, AND the rigid
// rope+robot sweep clears the floor Y rail (whose top sits at xr_z + yr_z).
des_half_x = usp_x/2 + 2;
function f_legs(h) = max(
      clear_fall + sag + h - xr_z - mount_rise - upper_h,
      sqrt(pow(sqrt(pow(sag + h, 2) + pow(des_half_x, 2)) + clear_swing, 2)
           - pow(axis_x - yr_x, 2)) + yr_z - mount_rise - upper_h);
function f_grise(h) = round((max([for (i = [0 : 2 : gantry_phi])
      -sag * cos(i) + sqrt(max(0, pow(h, 2) - pow(g_dx(i), 2)))]) + gantry_clear) / 10) * 10;
function f_gpost(h) = r5(f_grise(h) + mount_rise + gantry_h + post_cap);

// ---- the two orderable post sets --------------------------------------
// share_posts makes the SHORT build's leg the same stock length as the TALL
// build's gantry post, so four bars cover both roles. Lengthening the short
// build's leg only buys extra floor and swing clearance, never less.
gpost_tall  = f_gpost(hang_tall);
gpost_short = f_gpost(hang_short);
leg_tall    = r5(f_legs(hang_tall) - xr_z);
leg_short   = r5(max(f_legs(hang_short) - xr_z, share_posts ? gpost_tall : 0));
is_tall     = design_hang >= hang_tall;
leg_len     = is_tall ? leg_tall   : leg_short;
gpost_len   = is_tall ? gpost_tall : gpost_short;

legs_h  = auto_height ? leg_len + xr_z : legs_h_man;
deck_z  = (expo_mode ? 0 : legs_h) + xr_z;
frame_z = deck_z + upper_h;
axis_z  = frame_z + mount_rise;

gantry_rise    = gpost_len - mount_rise - gantry_h - post_cap;
gantry_z0      = axis_z + gantry_rise;
post_top_z     = show_gantry ? gantry_z0 + gantry_h + post_cap : frame_z;
deck_bot_z     = deck_z - xr_z;
pivot_deck_bot = frame_z - xr_z;
// ============================================================================

// ============ Frame profiles - v6.5 ============
module xrail(len) { color(c_profile) cube([len, xr_y, xr_z]); }
module yrail(len) { color(c_profile) cube([yr_x, len, yr_z]); }

// 30-series die-cast angle bracket, canonical frame:
//   leg1  u in [0,t], v in [0,w], w in [0,L]   - bolts to a face at u=0,
//                                                member behind it at u<0
//   leg2  u in [0,L], v in [0,w], w in [0,t]   - bolts to a face at w=0,
//                                                member below it at w<0
// Every instance below is placed by bracket_at(P,u,v,w) and the whole joint
// list was verified numerically (56/56: each leg lands on a real member face,
// nothing buried inside an extrusion) before being written here.
module diecast_bracket() {
    color(c_joint) difference() {
        union() {
            cube([bracket_t, bracket_w, bracket_L]);
            cube([bracket_L, bracket_w, bracket_t]);
            translate([0, (bracket_w + 5)/2, 0]) rotate([90, 0, 0])
                linear_extrude(5)
                    polygon([[0,0], [bracket_L*0.8,0], [0,bracket_L*0.8]]);
        }
        translate([-1, bracket_w/2, bracket_L/2]) rotate([0,90,0])
            cylinder(h = bracket_t + 2, d = bracket_bolt_d);
        translate([bracket_L/2, bracket_w/2, -1])
            cylinder(h = bracket_t + 2, d = bracket_bolt_d);
    }
}

module bracket_at(P, u, v, w) {
    multmatrix([[u[0], v[0], w[0], P[0]],
                [u[1], v[1], w[1], P[1]],
                [u[2], v[2], w[2], P[2]],
                [0, 0, 0, 1]]) diecast_bracket();
}

// A post face meeting an X beam face. zf = the beam face, up = the post rises
// from it (bracket above) or hangs below it (bracket below).
module post_beam_brackets(sx, sy, zf, up) {
    xf = sx ? frame_x - post_x : post_x;
    u  = [sx ? -1 : 1, 0, 0];
    by = sy ? frame_y - xr_y : 0;
    for (yo = [0, slot_pitch])                       // the beam's two T-slots
        bracket_at([xf, by + yo, zf], u, [0, 1, 0], [0, 0, up ? 1 : -1]);
}

// A Y rail end gets TWO brackets, one on the rail's top face and one on its
// bottom face. Separated by yr_z they form a moment couple instead of a
// hinge, which is what actually stiffens the Y-sway path - the alternative
// would be an end anchor bolt. Both land on the post (the top one) or on the
// X beam directly below it (the bottom one).
module yrail_post_brackets(zbot, ztop) {
    for (sx = [0, 1]) for (sy = [0, 1]) {
        x0 = sx ? frame_x - yr_x : 0;
        yf = sy ? frame_y - post_y : post_y;
        u  = [0, sy ? -1 : 1, 0];
        bracket_at([x0, yf, ztop], u, [1, 0, 0], [0, 0,  1]);
        bracket_at([x0, yf, zbot], u, [1, 0, 0], [0, 0, -1]);
    }
}

module frame_assembly() {
    // ================= THREE STAGES OF Z POST =================
    // stage 1 legs         floor beam top -> deck underside   (LAB only)
    // stage 2 mid posts    deck top       -> pivot beam under (always)
    // stage 3 gantry posts pivot beam top -> gantry top       (with gantry)
    for (sx = [0, 1]) for (sy = [0, 1]) {
        px = sx ? frame_x - post_x : 0;
        py = sy ? frame_y - post_y : 0;

        if (!expo_mode)
            translate([px, py, xr_z])
                color(c_seg_leg) cube([post_x, post_y, deck_bot_z - xr_z]);
        translate([px, py, deck_z])
            color(c_seg_mid) cube([post_x, post_y, pivot_deck_bot - deck_z]);
        if (show_gantry)
            translate([px, py, frame_z])
                color(c_seg_top) cube([post_x, post_y, post_top_z - frame_z]);

        if (show_brackets) {
            if (!expo_mode) {
                post_beam_brackets(sx, sy, xr_z,      true);   // leg on floor beam
                post_beam_brackets(sx, sy, deck_bot_z, false); // leg under deck
            }
            post_beam_brackets(sx, sy, deck_z,        true);   // mid on deck
            post_beam_brackets(sx, sy, pivot_deck_bot, false); // mid under pivot beam
            if (show_gantry) {
                post_beam_brackets(sx, sy, frame_z,        true); // gantry post on pivot beam
                post_beam_brackets(sx, sy, gantry_z0 + xr_z, true); // gantry X rail top
            }
        }
    }

    // ---- FLOOR X beam pair (LAB): full depth, the legs stand on it ----
    if (!expo_mode) {
        translate([0, 0, 0])              xrail(frame_x);
        translate([0, frame_y - xr_y, 0]) xrail(frame_x);
    }

    // ---- DECK (cut plane 1) and PIVOT BEAM (cut plane 2) ----
    translate([0, 0, deck_bot_z])                   xrail(frame_x);
    translate([0, frame_y - xr_y, deck_bot_z])      xrail(frame_x);
    translate([0, 0, pivot_deck_bot])               xrail(frame_x);
    translate([0, frame_y - xr_y, pivot_deck_bot])  xrail(frame_x);

    // ---- Y rails: 3060 ON EDGE, butting the posts. LAB -> floor,
    //      EXPO -> relocated onto the deck. Never between deck and pivot.
    yz = expo_mode ? deck_z : xr_z;   // clears room for the bottom bracket
    translate([0, post_y, yz])              yrail(yrail_len);
    translate([frame_x - yr_x, post_y, yz]) yrail(yrail_len);
    if (show_brackets) yrail_post_brackets(yz, yz + yr_z);

    // ---- top stiffening gantry (leaves together with stage 3) ----
    if (show_gantry) {
        translate([post_x, 0, gantry_z0])              xrail(xrail_len);
        translate([post_x, frame_y - xr_y, gantry_z0]) xrail(xrail_len);
        translate([0, post_y, gantry_z0])              yrail(yrail_len);
        translate([frame_x - yr_x, post_y, gantry_z0]) yrail(yrail_len);
        if (show_brackets) yrail_post_brackets(gantry_z0, gantry_z0 + yr_z);
    }
}
...
module top_pivot_bearing_mount(cutaway=show_bearing_cutaway) {
    plate_w = 58;
    plate_h = 52;
    flange_y = -bearing_center_to_rail - xr_slot_in;
    web_y    = -bearing_center_to_rail + (xr_y - xr_slot_in) - plate_t/2;
...
module rear_pivot_encoder_mount() {
    ...
    difference() {
        ...
        for (xoff = [-enc_rail_bolt_x, enc_rail_bolt_x])
            translate([axis_x + xoff, top_mount_rear_y,
...
module robot_upper_body_b_part() {
    // v5.5 bodyB = A/B-final equipment bay, DOOR RETIRED:
    ...
    //   - v5.5 X-ONLY OPEN FACE (CoM request): v5.4's three-side bay
    //     pushed the shell CoM too far to +X, so both Y walls are
    //     RESTORED to full section and only the X- face is open, at
    //     the full interior width (85 in the 90 zone vs the 75 board,
    //     93 in the head - v5.3 widths, no lips). The closed box
    //     section (Y walls + X+ wall + bulkhead) is back,
    //   - v5.5 SIDE-RAIL SHELF: the battery shelf keeps only two
    //     side rails along the Y walls; the whole middle is OPEN -
    //     that opening is the SWITCH BAY (power toggle sits there,
    //     reached from the front) and a straight wiring drop from the
    //     battery to the board top edge. Straps move onto the rails
    //     (one slot pair per rail), the front-edge toggle slot is
    //     retired,
    ...
    //   - battery on the rails at the very top in the flared (98) head;
    //     the 84 mm pack butts on the Y- wall, leads bend in the 8.5 mm
    //     Y+ lead bay, charge lead out the charge window, pack in along
    //     X through the open face.
    // Assembly: sleeve into A (2 cross bolts) -> board in through the
    // open face onto the ledge, tape to the pad -> wires (extra cables
    // land on the board top edge, 25 mm room; switch in the open shelf
    // middle) -> battery in along X onto the rails, strap per rail.
    // Print UPRIGHT as placed, sleeve down (~221 tall - fits X1C 256).
    // The shelf rails bridge ~30 mm across X - allow bridging or a
    ...
    open_w_body = usp_y_wide - 2*usp_wall_y - 2*lip_ret_y;   // 85
    ...
    difference() {
        union() {
            ...
            // v5.5 battery SIDE RAILS (full shelf plate retired): two
            // rails welded into the Y walls, front edge at the open
            // face line; the middle between them is the open switch
            // bay / wiring drop.
            for (sy = [-1, 1])
                translate([-usp_x/2 + usp_wall_x,
                           sy > 0 ? usp_y_head/2 - 1 - shelf_rail_w
                                  : -usp_y_head/2 + 1,
                           shelf_z0])
                    cube([usp_x - usp_wall_x - 3, shelf_rail_w, shelf_t]);
        ...
        // v5.5 X-ONLY OPEN FACE: remove the whole X- wall over the bay
        // across the full interior width (no lips - see lip_ret_y);
        // both Y walls stay at full section. 90 zone opening 85 (vs
        // the 75 board); flare/head zone opening 93.
        ...
        // v5.5 battery strap slots, ONE PAIR PER RAIL (the old central
        // pair lived in the removed shelf middle): each strap wraps
        // the pack end + the rail strip between its two slots (across
        // X, buckle on the open-face side, strap passes under the pack
        // through the open middle).
        for (sy = [-1, 1]) for (sx = [-1, 1])
            translate([sx*13,
                       sy*(usp_y_head/2 - 1 - shelf_rail_w/2),
                       shelf_z0 + shelf_t/2])
...
module full_assembly() {
    ...
    if (show_pivot_axis) {
        ...
        translate([axis_x, post_y/2, axis_z])
            cyl_y(frame_y - post_y, 2.2, center=false);
    }
...
if (is_undef(part_mode) || part_mode == "assembly") {
    ...
    // -------------------- Console summary --------------------
    echo("============================================================");
    echo("frame_master_v6.scad v6.4: 3060 posts, THREE Z stages, die-cast brackets, gantry");
    ...
    echo(str("Inverted hang (alpha=180, phi=0) floor clearance [mm] = ",
             axis_z - sag - usp_z1));
    robot_half_x = usp_x/2 + 2;   // bodyB open face + wall + taped wiring
    robot_hang_len = usp_z1;      // ankle -> head cap top
    rigid_corner_r = sqrt(pow(sag + robot_hang_len, 2) + pow(robot_half_x, 2));

    echo("------------------------- v6.2 FRAME -------------------------");
    echo(str("POSTS  ", post_x, " x ", post_y,
             " (3060, 60 along Y)  |  X rails ", xr_y, " x ", xr_z,
             " (3060 flat)  |  Y rails ", yr_x, " x ", yr_z, " (3060 on edge)"));
    echo(str("BEARING centre spacing [mm] = ", top_mount_rear_y - top_mount_front_y,
             "  must stay ", prof_ctr_y, " (carbon tube)  -> ",
             (top_mount_rear_y - top_mount_front_y == prof_ctr_y) ? "OK" : "*** MISMATCH ***"));
    echo(str("  pivot bolt line sits on the INNER T-slot, ", xr_slot_in,
             " mm in from the rail edge (no slot on the 60 mm centreline)",
             " -> frame_y = ", frame_y, ", Y rail = ", yrail_len));
    echo(str("  all cut lengths rounded UP to 5 mm; leg driven by ",
             (f_legs(design_hang) - xr_z) > (clear_fall + sag + design_hang
              - xr_z - mount_rise - upper_h - xr_z)
             ? "the SWING clearance to the floor Y rail" : "the floor clearance"));
    echo(str("Mode = ", expo_mode ? "EXPO (legs off)" : "LAB",
             "  |  deck underside z=", deck_bot_z, " top z=", deck_z,
             "  |  frame_z=", frame_z, "  axis_z=", axis_z));
    echo(str("Two cut planes: DECK z=", deck_bot_z, "..", deck_z,
             " and PIVOT BEAM z=", pivot_deck_bot, "..", frame_z,
             ". Every joint is a die-cast angle bracket ", bracket_L, "x",
             bracket_w, "x", bracket_t, " onto an X beam face - no butt splices."));
    echo(str("Y rails are at ", expo_mode ? "the DECK (short build)" :
             "the FLOOR (lab build)",
             " - the deck-to-pivot volume is left completely open in X."));

    // ---- what can the swinging robot still hit? ----
    yb_z = (expo_mode ? deck_z : xr_z) + yr_z;    // top face of the Y rails
    yrail_r = sqrt(pow(axis_x - yr_x, 2) + pow(axis_z - yb_z, 2));
    echo(str("Y-rail inner corner is ", yrail_r,
             " mm from the pivot vs rigid sweep ", rigid_corner_r,
             " mm -> clearance ", yrail_r - rigid_corner_r, " mm"));
    echo(str("Inverted-hang floor clearance [mm] = ",
             axis_z - sag - robot_hang_len));

    // ---- gantry sizing: head height rises with phi ----
    head0 = axis_z - sag + usp_z1;
    headp = axis_z - sag * cos(gantry_phi) + usp_z1;
    echo(str("Standing head top: phi=0 -> ", head0, " mm | phi=", gantry_phi,
             " -> ", headp, " mm (ankle rises ", sag * (1 - cos(gantry_phi)),
             " mm) -> gantry set at ", gantry_z0, " = headp + ", gantry_clear));
    gantry_r = sqrt(pow(axis_x - yr_x, 2) + pow(gantry_z0 - axis_z, 2));
    echo(str("Gantry Y rail: +", gantry_rise, " above pivot (mode-independent) = z ",
             gantry_z0, " | post top ", post_top_z));
    echo(str("  sized by the REACH ENVELOPE: robot of ", gantry_len,
             " mm, any lean, swing up to +/-", gantry_phi, " deg, margin ",
             gantry_clear, " mm"));
    echo("  required gantry z vs the swing angle you want covered:");
    for (ph = [30, 45, 60, 75, 90])
        echo(str("    phi +/-", ph, " deg -> z >= ", axis_z + max([for (i=[0:2:ph]) g_rise(i)]) + gantry_clear,
                 "   (post top ", axis_z + max([for (i=[0:2:ph]) g_rise(i)]) + gantry_clear + gantry_h + post_cap, ")"));
    echo(str(usp_z1 > design_hang
             ? str("  *** the previewed robot (", usp_z1,
                   ") is TALLER than design_hang (", design_hang, ") ***")
             : "  preview robot fits inside design_hang"));
    echo("  --- SWAPPABLE POST SETS -------------------------------------");
    echo("  Only stage 1 and stage 3 depend on the robot length. Stage 2, every");
    echo("  X beam, both Y rails and all 64 brackets are COMMON to both builds.");
    echo(str("    A  robot ", hang_short, ": leg x4 = ", leg_short,
             " | gantry post x4 = ", gpost_short,
             "   axis_z ", leg_short + 2*xr_z + upper_h + mount_rise));
    echo(str("    B  robot ", hang_tall, ": leg x4 = ", leg_tall,
             " | gantry post x4 = ", gpost_tall,
             "   axis_z ", leg_tall + 2*xr_z + upper_h + mount_rise));
    echo(str("    shared: A leg == B gantry post == ", leg_short,
             (leg_short == gpost_tall) ? "  -> ONE stock length" : "  *** not shared ***"));
    echo(str("    COMMON: mid post x4 = ", pivot_deck_bot - deck_z,
             " | X beam x6 = ", frame_x, " | X rail x2 = ", xrail_len,
             " | Y rail x4 = ", yrail_len));
    echo(str("    DISTINCT BARS TO ORDER: ", leg_short, " x4, ", gpost_short,
             " x4, ", leg_tall, " x4, ", pivot_deck_bot - deck_z, " x4  (16 posts)"));
    echo("  -------------------------------------------------------------");
    echo(str("  PREVIEW robot is ", robot_hang_len, " mm (robot_l1 ", robot_l1,
             " + robot_l2 ", robot_l2, ") but the frame is sized for design_hang ",
             design_hang, " - set robot_l1/robot_l2 to preview the design robot"));

    // ---- headroom if the robot grows ----
    for (hang = [robot_hang_len, 800, 1000]) {
        rr = sqrt(pow(sag + hang, 2) + pow(robot_half_x, 2));
        echo(str("  robot hang ", hang, " mm -> floor ",
                 axis_z - sag - hang, " mm | Y-rail ", yrail_r - rr,
                 " mm | gantry needs z >= ",
                 axis_z - sag * cos(gantry_phi) + hang + gantry_clear));
    }
    echo(str("DESIGN TARGETS: hang ", design_hang, " mm robot | standing ",
             clear_stand, " mm | fallen ", clear_fall, " mm"));
    echo(str("  -> upper_h = ", upper_h, "   legs_h = ", legs_h,
             auto_height ? "   (auto)" : "   (manual)"));
    echo(str("  SHORT build, standing: lowest hardware at z ",
             xr_z + upper_h + mount_rise - sag - ankle_drop, " mm"));
    echo(str("  LAB build, fallen: head cap top at z ",
             axis_z - sag - design_hang, " mm (current ", robot_hang_len,
             " mm robot: ", axis_z - sag - robot_hang_len, " mm)"));
    echo("CUT LIST [mm] - three Z stages, both cut planes are an X beam pair:");
    echo(str("  stage 1 leg        x4 = ", deck_bot_z - xr_z, "   (LAB only, slate blue)"));
    echo(str("  stage 2 mid post   x4 = ", pivot_deck_bot - deck_z,
             "   (always, light grey)"));
    echo(str("  stage 3 gantry post x4 = ", post_top_z - frame_z,
             "   (with the gantry, sage)"));
    // counted straight off frame_assembly(): full-depth X beams at the floor,
    // deck and pivot beam; short X rails ONLY at the gantry; Y rails at the
    // floor/deck AND at the gantry.
    n_xbeam = (expo_mode ? 0 : 2) + 4;                 // 600 mm, full depth
    n_xrail = show_gantry ? 2 : 0;                     // 540 mm, between posts
    n_yrail = 2 + (show_gantry ? 2 : 0);               // 550 mm
    echo(str("  X beam full depth  x", n_xbeam, " = ", frame_x,
             "   (floor", expo_mode ? " -" : " 2", " + deck 2 + pivot beam 2)"));
    echo(str("  X rail between posts x", n_xrail, " = ", xrail_len,
             "   (gantry only - the floor pair is a full-depth X beam)"));
    echo(str("  Y rail             x", n_yrail, " = ", yrail_len,
             "   (floor/deck 2 + gantry 2)"));
    echo(str("  3060 TOTAL = ", n_xbeam*frame_x + n_xrail*xrail_len
             + n_yrail*yrail_len + 4*(leg_len + gpost_len + (pivot_deck_bot - deck_z)),
             " mm for this build | both post sets together = ",
             4*(leg_short + gpost_short + leg_tall + (pivot_deck_bot - deck_z))
             + 6*frame_x + 2*xrail_len + 4*yrail_len, " mm"));
    echo(str("  30-series die-cast brackets = ",
             8 * ((expo_mode ? 2 : 4) + (show_gantry ? 2 : 0))
             + 8 * (1 + (show_gantry ? 1 : 0)),
             "   (2 per post/beam joint + 2 per Y rail end, top+bottom couple)"));
    echo("KNOBS: design_hang / clear_stand / clear_fall (heights follow) | gantry_phi");
    echo("       expo_mode (short build)    | post_x,post_y (frame_y auto-tracks)");
    echo("------------------------------------------------------------");
    ...
    // bulkhead, tape pad, seating ledge and shelf rails show -
