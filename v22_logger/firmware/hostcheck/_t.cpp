 ...
 *       · FALL 에 α 도 기록(FALLA) · 루프·DXL 계측(# stat, t) — 안전장치는 넣지 않았다 (사용자 방침 9/5)
 * ============================================================================
 ...
 *      이름 값  파라미터 (w 로 목록): gam trig rel vrel dead dstep dlim rest alim sgn fdeg lock r lam c0 kv wmode wf wb p2r fphi fank loghz vel acc ilim
...
float FPHI = 0.0f, FANK = 0.0f;          // [fphi][fank] 직립 변환 +180 (0/1) — 매달림 영점을 잡고 세우면 그 채널이 180° 돈다 (브링업 §4-1). 앱 phi_off/ank_off 와 같은 뜻
...
int   err_code = 0;
// ---- 계측 (안전장치 아님, 숫자만 센다): 루프 주기 최대 · DXL 읽기 시간 최대 · 읽기 0반환 수 · 쓰기 실패 수 ----
uint32_t st_loop_max_us = 0, st_dxl_max_us = 0, st_prev_us = 0, st_next_ms = 0;  uint32_t st_dxl_fail = 0, st_wr_fail = 0;
void stReset() { st_loop_max_us = st_dxl_max_us = 0; st_dxl_fail = st_wr_fail = 0; }
void stPrint() {
  Serial.print("# stat loop_max "); Serial.print(st_loop_max_us / 1000.0f, 1); Serial.print(" ms  dxl_max "); Serial.print(st_dxl_max_us / 1000.0f, 1);
  Serial.print(" ms  dxl_fail "); Serial.print(st_dxl_fail); Serial.print("  wr_fail "); Serial.println(st_wr_fail);
}
...
void writeGoal(float deg) {
  if (motor_ok && !dry_run) { if (!dxl.setGoalPosition(DXL_ID, home_tick + MOTOR_DIR * deg * TICK_PER_DEG)) st_wr_fail++; }
}
float readDelta() {
  if (!motor_ok) return hold;
  uint32_t us0 = micros();
  float t = dxl.getPresentPosition(DXL_ID);
  uint32_t took = micros() - us0; if (took > st_dxl_max_us) st_dxl_max_us = took;
  if (t == 0.0f) { err_code |= 16; st_dxl_fail++; return delta_now; }
  dxl_raw = t;
...
void readState() {
  ...
  if (FPHI > 0.5f) phi_d = wrap180(phi_d + 180.0f);          // 매달림 영점 → 직립: 그 채널은 180° 돌아 있다
  if (FANK > 0.5f) ank_d = wrap180(ank_d + 180.0f);
  delta_now = readDelta();
  alpha_d = wrap180(ank_d - phi_d);
...
void logHeader() {
  ...
  Serial.println("# v22_raw v3: phase 0 IDLE/1 FOLD/2 REST/3 STOP/4 대기. cue=단일접기 완료. err = phi등급 + 4·ank등급 + 16·dxl");
}
void logLine() {
  Serial.print("D,");
...
void printStatus() {
  ...
  Serial.print("직립 변환 fphi "); Serial.print((int)FPHI); Serial.print("  fank "); Serial.println((int)FANK);
  stPrint();
}
...
const Param PARAMS[] = {
  ...
  {"fphi", &FPHI,       0.0f,   1.0f, "φ 직립 변환 +180 (0/1) — 매달림 영점 뒤 세우면"},
  {"fank", &FANK,       0.0f,   1.0f, "발목 직립 변환 +180 (0/1)"},
...
void doGo() {
  ...
  if (zero_stage == 0) Serial.println("# ⚠ 영점 없음 (단계 0/2) — 각도가 엔코더 원점 기준이다");
  if (motor_ok && !dry_run) {
    // 토크가 꺼진 채 무장하면 접기 명령이 허공에 간다 (v2 는 STOPPED 뒤에만 켰다 — hangcal torqueRestoreHere 처럼 항상 보장)
    if (!dxl.getTorqueEnableStat(DXL_ID)) {
      float pos = dxl.getPresentPosition(DXL_ID);
      if (pos == 0.0f) { Serial.println("# !! 모터 위치를 못 읽어 GO 안 함 (home 이 0 이 돼 튄다) — 12V·4핀 커넥터·보드레이트, t 의 dxl_fail"); return; }
      home_tick = pos; hold = 0; dxl.setGoalPosition(DXL_ID, home_tick); dxl.torqueOn(DXL_ID);
      Serial.println("# 토크 ON (현재 자세 = δ 0 재정의)");
    }
    delta_now = readDelta();
  }
  hold = delta_now; primed = false; dphi = dbeta = 0;
  once_done = false; post_pending = false; phase = IDLE; phase_t0 = millis();
  stReset(); st_next_ms = millis() + 5000;
  running = true;
...
void setup() {
  ...
  Serial.println("# v22_raw v3 — z u k <정수> m s p t w e swap hdr | mode N · g · h · x · y · fold X | 이름 값 (gam trig r lam c0 kv fphi fank …)");
  logHeader();
...
void loop() {
  ...
  { uint32_t nowus = micros(); if (st_prev_us) { uint32_t per = nowus - st_prev_us; if (per > st_loop_max_us) st_loop_max_us = per; } st_prev_us = nowus; }
  readState();
  if (running && (fabsf(phi_d) > ANG_LIMIT || fabsf(alpha_d) > ANG_LIMIT)) {
    torqueOffAll("한계각 초과 (넘어짐) — k 또는 g 로 복구");
    emitE("FALL", phi_d, 2); emitE("FALLA", alpha_d, 2);            // 어느 쪽이 걸렸는지 로그로 (φ 인지 α 인지)
    stPrint();
  }
  if (running) controlStep();
  uint32_t ms = millis();
  if (running && (int32_t)(ms - st_next_ms) >= 0) { st_next_ms = ms + 5000; stPrint(); st_loop_max_us = st_dxl_max_us = 0; }   // 5 s 마다 계측 한 줄 (실패 수는 GO 이후 누적)
