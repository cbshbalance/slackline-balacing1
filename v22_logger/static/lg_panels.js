  ...
  function renderResult(res) {
    ...
    if (res.next) {
      ...
      h = `<div style="border:1px solid var(--cyan);border-radius:8px;padding:8px 10px;margin-bottom:8px;background:rgba(76,201,240,.08)">
        ...
        <div style="font-family:Consolas;font-size:15px;color:#fff;margin:3px 0">β <b>${(+nx.beta).toFixed(2)}</b>°   φ <b>${(+nx.phi).toFixed(2)}</b>°   (ank ${(+nx.ank).toFixed(2)}°)  ${nx.side > 0 ? "선 위쪽" : "선 아래쪽"}${nx.off != null ? " " + (+nx.off).toFixed(2) + "°" : ""}</div>
