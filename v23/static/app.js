...
function setSession(data){session=data;rows=data.rows||[];cursor=0;playing=false;$("analysisResult").textContent='';$("follow").checked=false;$("scrub").max=Math.max(0,rows.length-1);$("sessionName").textContent=data.name||'';$("metadata").textContent=JSON.stringify({kind:data.kind,config:data.config,plane:data.plane,inputs:data.inputs,hashes:data.hashes,notes:data.notes},null,2);if(data.geom){LG.GEOM=data.geom;for(const fn of listeners.hello||[])fn(data);}renderEvents();update();}
